# Myrkfell — world plan

> Written by the in-game god editor (`scripts/GodEditor.gd`) on 2026-09-11T15:51:05.
> Data: `res://design/world_plan.json`. Edit the world in game, not this file — it is regenerated on every save.

Coordinates are world metres: **+x east, +z south**, y up. Sea level is y −22.5,
the spawn valley is the origin. `Overworld.ground_y(pos)` is the surface.

## Open notes (1)

- [ ] **bedroll** `todo` · `(2, 0, 2)` · near **Portland** (temperate)

    make the bedroll not spawn on the ground

## Zones (0)

_None drawn yet._

## Roads, trails and lines (0)

_None drawn yet._

## Loose notes (1)

Pins dropped outside any zone or path.

- [ ] **bedroll** `todo` · `(2, 0, 2)` · near **Portland** (temperate)

    make the bedroll not spawn on the ground

---

### Legend

`[ ]` open · `[x]` done · **status** idea → planned → building → done ·
**priority** low / normal / high / next.
Tags: `todo` `idea` `question` `bug` `lore` `done`.

