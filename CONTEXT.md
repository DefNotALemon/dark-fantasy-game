...
- **4 War Axe** — heavy one-handed cleaver (`_try_axe_swing`/`_update_axe`/`_do_axe_hit`): ×1.35 base damage, slower, no combo ladder — two ALTERNATING authored swings (overhead chop / left-to-right cleave), impact-timed damage, hits everything in the arc, LOS-checked. No material matchups yet (TODO: fold into metal system at step 4/5). Can't block with it; offhand shield/torch stay up. **CHOPS TREES** (`_chop_tree`/`_fell_tree`): trees are group `"trees"` with `chops`/`height` metas (World._make_tree); each bite bursts wood-chip debris (`RockDebris` wood variant) + trunk shiver; last bite = collision off, eased tip-over away from the axe, canopy crash (chips + proximity shake), sink + free, yields 2-4 Wood items.
