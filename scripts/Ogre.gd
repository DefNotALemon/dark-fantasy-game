...
func _animate(delta: float) -> void:
	...
	if strong_windup > 0.0:
		...
		if strong_mode == "bite":
			...
			arm.rotation_degrees = arm.rotation_degrees.lerp(Vector3(-35.0 * p, 0, 0), delta * 16.0)  ## reach out to grab
		else:
	...
	if strong_active:
		...
		if strong_mode == "bite":
			jaw.rotation_degrees = Vector3(lerpf(35.0, 0.0, minf(p * 2.2, 1.0)), 0, 0)
			rig.rotation_degrees = Vector3(lerpf(-10.0, 8.0, minf(p * 2.0, 1.0)), 0, 0)
		elif strong_mode == "throw":
			## Sweep the arm up and over — hurling you across its shoulder.
