...
func _animate(delta: float) -> void:
	...
	if strong_windup > 0.0:
		...
		if strong_mode == "pounce":
			## Coil down low, club dragged back.
	...
	if strong_active:
		...
		if strong_mode == "pounce":
			## Stretched out mid-leap, club coming down overhead.
			rig.position.y = lerpf(-0.16, 0.06, minf(p * 2.0, 1.0))
			rig.rotation_degrees = Vector3(lerpf(14.0, -10.0, p), 0, 0)
			arm.rotation_degrees = Vector3(lerpf(40.0, -110.0 + 150.0 * p, p), 0, 0)
		else:
	...
	if melee_anim > 0.0:
		...
		if p < 0.4:
			...
			arm.rotation_degrees = Vector3(lerpf(-75.0, 35.0, (p - 0.4) / 0.6), 0, 0)
		return
