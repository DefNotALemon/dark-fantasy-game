...
func _animate(delta: float) -> void:
	...
	if strong_windup > 0.0:
		...
		if strong_mode == "lunge":
			## Drop low with the spear pulled BACK (negative x = fist drawn
			## behind the hip, tip low — coiled to dart).
			...
			arm.rotation_degrees = arm.rotation_degrees.lerp(Vector3(-35.0 * p, 0, 0), delta * 16.0)
		else:
	...
	if strong_active:
		...
		if strong_mode == "lunge":
			## Spear RAMMED out forward at chest height while darting in.
			## (The old signs were mirrored — the thrust whipped the spear up
			## BEHIND its own back while the body flew at you.)
			rig.position.y = -0.12
			rig.rotation_degrees = Vector3(12.0, 0, 0)
			arm.rotation_degrees = Vector3(lerpf(-35.0, 20.0, minf(p * 2.5, 1.0)), 0, 0)
		else:
			## Whirl a full turn, tail out.
			rig.rotation_degrees = Vector3(0, lerpf(-45.0, 315.0, p), 0)
		return
	if melee_anim > 0.0:
		## The jab: snatch the spear back, then RAM it out forward — at full
		## extension right on the damage frame (p≈0.62); the rest lerp below
		## reels it back in. (Old signs were mirrored: it poked at the sky and
		## then "stabbed" up behind its own shoulder.)
		var p := 1.0 - melee_anim / MELEE_ANIM_TIME
		if p < 0.30:
			arm.rotation_degrees = arm.rotation_degrees.lerp(Vector3(-30, 0, 0), delta * 24.0)
		else:
			arm.rotation_degrees = Vector3(lerpf(-30.0, 20.0, minf((p - 0.30) / 0.32, 1.0)), 0, 0)
		return
