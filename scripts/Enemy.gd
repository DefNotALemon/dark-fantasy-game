...
## --- SKELETON, RAGDOLL, MASS (CreatureSkin) ---
## After _build_body the box rig is baked into a Skeleton3D + one skinned PSX
## mesh. `skin` owns the bones, the ragdoll, and the shove-by-mass rules.
const RAGDOLL_DEATH := true      ## corpses fall over instead of shattering
@export var mass := 60.0         ## kg-ish; decides who shoves whom (see CreatureSkin)
var skin: CreatureSkin = null
var skin_tile := "skin"          ## default PsxTex tile for this body
var knocked := false             ## ragdolled but alive — gets back up
var _last_hit_dir := Vector3.ZERO   ## where the last blow was pushing us (death fling)

## --- FLOW: the layer between standing and acting ---
## Two things every body does that no authored pose covers: it LEANS — into
## acceleration, into a turn — and, left alone, it FIDGETS. Both are written
## as small offsets ON TOP of whatever _animate left, applied after it and
## removed again before the next frame's locomotion, so the authored poses
## never see them and nothing accumulates. The skin (which ticks after us)
## calls _update_flow; _update_locomotion strips last frame's offsets.
var _flow_applied: Array = []       ## [[node, rot_deg_offset, pos_offset]]
var _flow_ready := false            ## locomotion ran this frame, offsets are clear
var _prev_vel := Vector3.ZERO
var _prev_yaw := 0.0
var _lean := Vector2.ZERO           ## smoothed (pitch, bank) in degrees
var _fid_kind := ""                 ## current fidget
var _fid_t := 0.0
var _fid_len := 1.0
var _fid_cool := randf_range(1.0, 4.0)
var _fid_side := 1.0
var _breath_t := randf() * TAU


func _ready() -> void:
	add_to_group("enemies")
	## Layers: creatures live on layer 2 (the world is 1). Ragdoll bones sit
	## on layer 8 and look only at the world and each other, so a charging
	## moose runs THROUGH a downed goblin instead of bulldozing its corpse
	## twenty metres down the road. See CreatureSkin.RAGDOLL_LAYER.
	collision_layer = 2
	collision_mask = 1 | 2
	health = max_health
	_build_body()
...
func _skin_opts() -> Dictionary:
	## What the skin should know: the default texture tile for this body and
	## its mass. Subclasses override (Critter derives both from the dex).
	return {"tile": skin_tile, "mass": mass}


## ---------------------------------------------------------- knockdowns ---

func knockdown(fling: Vector3 = Vector3.ZERO, seconds := 2.4) -> void:
	## Off its feet: a trample, a charge, a falling tree. The body goes to the
	## ragdoll, the brain stops, and it gets up where it lands.
	if dying or knocked or skin == null or (armored and fling.length() < 6.0):
		return
	knocked = true
	strong_active = false
	strong_windup = 0.0
	...
	skin.ragdoll_start(fling, seconds, false)


func _on_ragdoll_up() -> void:
	## The skin finished its knockdown clock: stand up where the pelvis is.
	var where: Dictionary = skin.ragdoll_stop() if skin.ragdoll else {}
	if not where.is_empty():
		var pos: Vector3 = where["pos"]
		## drop the root to the floor under the pelvis so it stands on the ground
		var space := get_world_3d().direct_space_state
		if space:
			var q := PhysicsRayQueryParameters3D.create(pos + Vector3.UP * 0.5, pos + Vector3.DOWN * 3.0)
			q.exclude = [get_rid()]
			q.collision_mask = 1
			...
			if not hit.is_empty():
				pos = hit.position
		global_position = pos
		rotation = Vector3(0.0, float(where["yaw"]), 0.0)
	for c in get_children():
		if c is CollisionShape3D:
			c.set_deferred("disabled", false)
	knocked = false
	flinch_timer = maxf(flinch_timer, 0.45)   ## a beat on its feet before it acts


func _knocked_tick(_delta: float) -> void:
	## Down. Nothing to steer; the ragdoll clock lives in the skin. Some skins
	## never built (no boxes) — never strand a creature in a state it cannot
	## leave.
...
static func _cut_arc(p: float, carry: float, chamber: float, through: float) -> float:
	...
	## SIGN NOTE for every pose in every subclass (the backward-swing disease —
	## do not reinfect): arms hang along -y and mobs face -z, so POSITIVE
...
func _flow_unapply() -> void:
	for it in _flow_applied:
		var n := it[0] as Node3D
		if n != null and is_instance_valid(n):
			n.rotation_degrees -= it[1] as Vector3
			n.position -= it[2] as Vector3
	_flow_applied.clear()


func _flow_add(n: Node3D, rot: Vector3, pos := Vector3.ZERO) -> void:
	if n == null:
		return
	n.rotation_degrees += rot
	n.position += pos
	_flow_applied.append([n, rot, pos])


func _flow_node() -> Node3D:
	if loco_root != null:
		return loco_root
	var r = get("rig")
	return r as Node3D if r is Node3D else null


func _is_acting() -> bool:
	return strong_windup > 0.0 or strong_active or melee_anim > 0.0 or flinch_timer > 0.0 \
		or climbing or routing or retreat_timer > 0.0


func _update_flow(delta: float) -> void:
	## Called by the CreatureSkin after _animate. Lean + fidget, additive.
	if not _flow_ready:
		_flow_unapply()   ## locomotion did not run (asleep, down): leave nothing behind
		return
	_flow_ready = false
	if dying or knocked:
		return
	...
	if body == null:
		return
	## ---- LEAN: acceleration pitches the body, turning banks it
	var acc := (velocity - _prev_vel) / maxf(delta, 0.0001)
	_prev_vel = velocity
	acc.y = 0.0
	...
	var speed_k := clampf(_loco_amount * 1.5, 0.0, 1.0)
	var pitch := -clampf(acc.dot(fwd) * 0.45, -9.0, 9.0)             ## nose down into a sprint start
	var bank := clampf(rad_to_deg(yaw_rate) * 0.035 * speed_k, -11.0, 11.0) \
		- clampf(acc.dot(right) * 0.35, -6.0, 6.0)                       ## into the turn
	var target := Vector2(pitch, bank)
	_lean = _lean.lerp(target, clampf(delta * 7.0, 0.0, 1.0))
	## ---- BREATH: everything with lungs
	_breath_t += delta
	var breath := 0.55 * sin(_breath_t * 1.7) * (1.0 - _loco_amount * 0.5)
	_flow_add(body, Vector3(_lean.x + breath, 0.0, _lean.y))
	## ---- FIDGETS: only while standing about (the wildlife has its own buzz)
	if self is Critter:
		return
	if _is_acting() or _loco_amount > 0.08:
		_fid_kind = ""
		_fid_cool = maxf(_fid_cool, 0.6)
		return
	if _fid_kind == "":
		_fid_cool -= delta
		if _fid_cool <= 0.0:
			_fid_cool = randf_range(1.4, 4.5)
			var kinds: Array = ["shift", "shift", "look", "look", "stomp", "arms", "roll"]
			if walk_legs.is_empty():
				kinds.erase("stomp")
			if walk_arms.is_empty():
				kinds.erase("arms")
			_fid_kind = String(kinds[randi() % kinds.size()])
			_fid_t = 0.0
			_fid_len = float({"shift": 2.4, "look": 2.8, "stomp": 0.9, "arms": 1.6, "roll": 1.3}[_fid_kind])
			_fid_side = 1.0 if randf() < 0.5 else -1.0
		return
	_fid_t += delta
	var t := clampf(_fid_t / _fid_len, 0.0, 1.0)
	var env := sin(t * PI)   ## in and out
	match _fid_kind:
		"shift":
			## weight onto one hip: body slides and rolls a touch, one knee softens
			_flow_add(body, Vector3(0.0, 0.0, -3.5 * env * _fid_side), Vector3(0.035 * env * _fid_side, -0.012 * env, 0.0))
			if walk_legs.size() >= 2:
				var soft := walk_legs[0 if _fid_side > 0.0 else 1]
				_flow_add(soft, Vector3(-6.0 * env, 0.0, 0.0))
		"look":
			## glance to one side, hold, come back
			var e := _ease_hold(t)
			_flow_add(body, Vector3(0.0, 24.0 * e * _fid_side, 1.5 * e * _fid_side))
		"stomp":
			## one foot lifts and plants, the body dips on the plant
			if not walk_legs.is_empty():
				var leg := walk_legs[0 if _fid_side > 0.0 else (1 if walk_legs.size() > 1 else 0)]
				var lift := sin(clampf(t / 0.6, 0.0, 1.0) * PI)
				_flow_add(leg, Vector3(-22.0 * lift, 0.0, 0.0))
				if t > 0.6:
					var plant := (1.0 - t) / 0.4
					_flow_add(body, Vector3(1.5 * plant, 0.0, 0.0), Vector3(0.0, -0.02 * plant, 0.0))
		"arms":
			## loose arm swings once, a shrug of the shoulders
			for a in walk_arms:
				_flow_add(a, Vector3(14.0 * sin(t * TAU) * env, 0.0, 6.0 * env))
			_flow_add(body, Vector3(-1.0 * env, 0.0, 0.0), Vector3(0.0, 0.012 * env, 0.0))
		"roll":
			## roll the shoulders / crack the neck
			_flow_add(body, Vector3(2.5 * env, 5.0 * sin(t * TAU) * env, 0.0))
	if t >= 1.0:
		_fid_kind = ""


static func _ease_hold(t: float) -> float:
	## 0 -> 1 over the first quarter, hold, back to 0 over the last quarter.
	if t < 0.25:
		var u := t / 0.25
...
func _die() -> void:
	...
	if RAGDOLL_DEATH and skin != null and skin.bone_count() > 1:
		## It falls over. The ragdoll takes the last blow's direction, settles,
		## lies there a while, and dithers away (CreatureSkin owns the clock and
		## frees us at the end). Loot drops as the body hits the ground.
		knocked = false
